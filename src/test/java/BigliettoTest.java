...
public class BigliettoTest {
    ...
    @Test
    public void testApplicazionePromozione() {
        PromozioneImplDB promozioneDB = PromozioneImplDB.getInstance();
        String promoID="PROMO_NO_" + System.currentTimeMillis();
        Promozione promozione = new Promozione.PromozioneBuilder()
                .setPromozioneID(promoID)
                .setTreno(treno)
                .setTratta(tratta)
                .setDataPartenza("2025-12-01")
        ...
        promozioneDB.removePromozionePID(promoID);
    }

    @Test
    public void testPromozioneNonApplicabile() {
        PromozioneImplDB promozioneDB = PromozioneImplDB.getInstance();
        String promoID="PROMO_NO_" + System.currentTimeMillis();
        Promozione promozione = new Promozione.PromozioneBuilder()
                .setPromozioneID(promoID)
        ...
        double prezzoOriginale = biglietto.getPrezzo();
        double prezzoFinale = ApplicaPromozione.Promozione(biglietto);
        System.out.println("I test potrebbero non funzionare in presenza di altre promozioni simili, TreniCal prende la prima promozione applicabile");
        assertEquals(prezzoOriginale, prezzoFinale, 0.01);
